...
public class PostgresPhysicalAssetDescriptionNotificationService {
    ...
    public Optional<PhysicalAssetDescriptionNotificationRecord> getLastRecord() throws StorageException {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql);
            ...
            if (rs.next()) {
                ...
                PhysicalAssetDescriptionNotificationRecord record = objectMapper.readValue(json, PhysicalAssetDescriptionNotificationRecord.class);
                return Optional.of(record);
            }
        } catch (SQLException e) {
            e.printStackTrace();
            throw new StorageException("Physical description notification SQL error receiving last record: " + e.getMessage());
        } catch (JsonProcessingException e) {
            e.printStackTrace();
            throw new StorageException("Physical description notification JSON processing error receiving last record: " + e.getMessage());
        }
    ...
    public List<PhysicalAssetDescriptionNotificationRecord> getRecordsInTimeRange(long startTimestampMs, long endTimestampMs) throws StorageException {
        List<PhysicalAssetDescriptionNotificationRecord> results = new ArrayList<>();
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            e.printStackTrace();
            throw new StorageException("Physical description notification SQL error receiving records in time range: " + e.getMessage());
        } catch (JsonProcessingException e) {
            e.printStackTrace();
            throw new StorageException("Physical description notification JSON processing error receiving records in time range: " + e.getMessage());
        }
        return results;
    }
    ...
    public List<PhysicalAssetDescriptionNotificationRecord> getRecordsInRange(int startIndex, int endIndex) throws StorageException {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            e.printStackTrace();
            throw new StorageException("Physical description notification SQL error receiving records in range: " + e.getMessage());
        } catch (JsonProcessingException e) {
            e.printStackTrace();
            throw new StorageException("Physical description notification JSON processing error receiving records in range: " + e.getMessage());
        }
        return results;
    }
