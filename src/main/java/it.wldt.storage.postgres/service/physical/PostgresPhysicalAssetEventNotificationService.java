...
public class PostgresPhysicalAssetEventNotificationService {
    ...
    public Optional<PhysicalAssetEventNotificationRecord> getLastRecord() throws StorageException {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql);
            ...
            if (rs.next()) {
                ...
                PhysicalAssetEventNotificationRecord record = objectMapper.readValue(json, PhysicalAssetEventNotificationRecord.class);
                return Optional.of(record);
            }
        } catch (SQLException e) {
            e.printStackTrace();
            throw new StorageException("Physical event SQL error receiving last record: " + e.getMessage());
        } catch (JsonProcessingException e) {
            e.printStackTrace();
            throw new StorageException("Physical event JSON processing error receiving last record: " + e.getMessage());
        }
    ...
    public List<PhysicalAssetEventNotificationRecord> getRecordsInTimeRange(long startTimestampMs, long endTimestampMs) throws StorageException {
        List<PhysicalAssetEventNotificationRecord> results = new ArrayList<>();
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            e.printStackTrace();
            throw new StorageException("Physical event SQL error receiving records in time range: " + e.getMessage());
        } catch (JsonProcessingException e) {
            e.printStackTrace();
            throw new StorageException("Physical event JSON processing error receiving records in time range: " + e.getMessage());
        }
        return results;
    }
    ...
    public List<PhysicalAssetEventNotificationRecord> getRecordsInRange(int startIndex, int endIndex) throws StorageException {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            e.printStackTrace();
            throw new StorageException("Physical event SQL error receiving records in range: " + e.getMessage());
        } catch (JsonProcessingException e) {
            e.printStackTrace();
            throw new StorageException("Physical event JSON processing error receiving records in range: " + e.getMessage());
        }
        return results;
    }
